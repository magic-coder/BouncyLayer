...
public class BouncyLayer extends FrameLayout implements SpringListener {
    ...
    private SlidingLayer.OnInteractListener mOnInteractListener;

    /**
     * Optional callback to notify client when scroll position has changed
     */
    private SlidingLayer.OnScrollListener mOnScrollListener;

    private int mMinimumVelocity;
    private int mFlingDistance;

    private LayerTransformer2 mLayerTransformer;

    /*Spring Animations*/
    private boolean springAnimate = false;
    private Spring mSpring;

    private int bounceAxis;

    public static final int BOUNCE_X_AXIS = 1;
    public static final int BOUNCE_Y_AXIS = 2;

    //For future use
    public static final int BOUNCE_DUAL_AXIS = 3;
    //Boundary1
    private int mBoundSlideRestX;
    private int mBoundSlideRestY;

    // Boundary for spring animation
    private boolean extendRestBoundary = false;
    private boolean extendFullBoundary = false;

    //Boundary2
    private int mBoundSlideFullX;
    private int mBoundSlideFullY;


    private int mSpringPreviousX;
    private int mSpringPreviousY;

    private int mSpringNextX;
    private int mSpringNextY;
    ...
    public BouncyLayer(Context context) {
        this(context, null);
    }

    public BouncyLayer(Context context, AttributeSet attrs) {
     ...
     */
    public BouncyLayer(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        // Style
        final TypedArray ta = context.obtainStyledAttributes(attrs, com.wunderlist.slidinglayer.R.styleable.SlidingLayer);

        // Set the side of the screen
        setStickTo(ta.getInt(com.wunderlist.slidinglayer.R.styleable.SlidingLayer_stickTo, STICK_TO_RIGHT));

        // Sets the shadow drawable
        int shadowRes = ta.getResourceId(com.wunderlist.slidinglayer.R.styleable.SlidingLayer_shadowDrawable, INVALID_VALUE);
        if (shadowRes != INVALID_VALUE) {
            setShadowDrawable(shadowRes);
        }

        // Sets the shadow size
        mShadowSize = (int) ta.getDimension(com.wunderlist.slidinglayer.R.styleable.SlidingLayer_shadowSize, 0);

        // Sets the ability to open or close the layer by tapping in any empty space
        changeStateOnTap = ta.getBoolean(com.wunderlist.slidinglayer.R.styleable.SlidingLayer_changeStateOnTap, true);

        // How much of the view sticks out when closed
        mOffsetDistance = ta.getDimensionPixelOffset(com.wunderlist.slidinglayer.R.styleable.SlidingLayer_offsetDistance, 0);

        // Sets the size of the preview summary, if any
        mPreviewOffsetDistance = ta.getDimensionPixelOffset(com.wunderlist.slidinglayer.R.styleable.SlidingLayer_previewOffsetDistance,
                INVALID_VALUE);
    ...
    /*
       Sliding layer moves back and forth from 'Rest' position to 'Full' position typically called as boundaries
       Spring animation is applied when
       @param extendRestBoundary True if the layer should go beyond rest boundary while performing spring animation. Default is false;
       @param extendFullBoundary True if the layer should go beyond rest boundary while performing spring animation. Default is false;
       @param bounceAxis This is the axis where the spring animation takes place.
                           @set BOUNCE_X_AXIS if spring should animate in x axis
                           @set BOUNCE_Y_AXIS if spring should animate in y axis
    */
    public void setSpringAnimation(double sprinTension, double springFriction, boolean extendRestBoundary, boolean extendFullBoundary){
        springAnimate = true;
        this.extendRestBoundary = extendRestBoundary;
        this.extendFullBoundary = extendFullBoundary;

        //Initialize spring only if spring animation is set
        BaseSpringSystem baseSpringSystem = SpringSystem.create();
        mSpring = baseSpringSystem.createSpring();
        mSpring.setSpringConfig(SpringConfig.fromOrigamiTensionAndFriction(sprinTension, springFriction));
        ...
        //Next positions are set when the animation is performed
    }
    ...
    private void setLayerState(final int state, final boolean smoothAnimation, final boolean force,
        ...
        if (smoothAnimation) {
            ...
            /*Spring Animations*/
            if(springAnimate && mSpring != null){

                mSpringPreviousX = mSpringNextX;
                mSpringPreviousY = mSpringNextY;

                mSpringNextX = pos[0];
                mSpringNextY = pos[1];

                if(mSpringNextX != mBoundSlideRestX){
                    mBoundSlideFullX = mSpringNextX;
                }
                if(mSpringNextY != mBoundSlideRestY){
                    mBoundSlideFullY = mSpringNextY;
                }
                ...
                if(bounceAxis == BOUNCE_X_AXIS){
                    mSpring.setCurrentValue(getScrollX());
                    mSpring.setEndValue(mSpringNextX);
                } else if(bounceAxis == BOUNCE_Y_AXIS){
                    mSpring.setCurrentValue(getScrollY());
                    mSpring.setEndValue(mSpringNextY);
                }
     ...
     * {@link SlidingLayer.OnInteractListener}.
     *
     ...
     * {@link SlidingLayer.OnScrollListener}.
     *
     * @param listener Listener to set
     */
    public void setOnScrollListener(SlidingLayer.OnScrollListener listener) {
     ...
     */
    public void setLayerTransformer(LayerTransformer2 layerTransformer) {
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        BouncyLayer.SavedState state = new BouncyLayer.SavedState(superState);
        if (mState == null) {
            mState = new Bundle();
        }
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        BouncyLayer.SavedState savedState = (BouncyLayer.SavedState) state;
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsDragging) {
                    ...
                    if(bounceAxis == BOUNCE_X_AXIS && x > (getWidth() - mOffsetDistance)){
                        break;
                    } else if(bounceAxis == BOUNCE_Y_AXIS && y > (getHeight() - mOffsetDistance)) {
                        break;
                    }
    ...
    /**
     * Sets the default location where the BouncyLayer will appear
     *
    ...
    public void onSpringUpdate(Spring spring) {
        ...
        int springIntValue = (int) springValue;

        //TODO: Restrict the scroll to the stop boundary if defined
        if(bounceAxis == BOUNCE_X_AXIS){
            BouncyLayer.this.scrollTo(springIntValue, mSpringNextY);
        } else if(bounceAxis == BOUNCE_Y_AXIS){
            BouncyLayer.this.scrollTo(mSpringNextX, springIntValue);
        } else {
            //This section is for dual axis bounce which can be considered in future
        }
    ...
    /**
     * Handler interface for obtaining updates on the <code>BouncyLayer</code>'s state.
     * <code>OnInteractListener</code> allows for external classes to be notified when the <code>BouncyLayer</code>
     * receives input to be opened or closed.
     */
    public interface OnInteractListener {

        /**
         * This method is called when an attempt is made to open the current <code>BouncyLayer</code>. Note
         * that because of animation, the <code>BouncyLayer</code> may not be visible yet.
         */
        void onOpen();

        /**
         * This method is called when an attempt is made to show the preview mode in the current
         * <code>BouncyLayer</code>. Note that because of animation, the <code>BouncyLayer</code> may not be
         * visible yet.
         */
        void onShowPreview();

        /**
         * This method is called when an attempt is made to close the current <code>BouncyLayer</code>. Note
         * that because of animation, the <code>BouncyLayer</code> may still be visible.
         */
        void onClose();
    ...
    static class SavedState extends BaseSavedState {
        ...
        public static final Creator<BouncyLayer.SavedState> CREATOR = new Creator<BouncyLayer.SavedState>() {
            @Override
            public BouncyLayer.SavedState createFromParcel(Parcel in) {
                return new BouncyLayer.SavedState(in);
            }

            @Override
            public BouncyLayer.SavedState[] newArray(int size) {
                return new BouncyLayer.SavedState[size];
            }
        };
    }
}
